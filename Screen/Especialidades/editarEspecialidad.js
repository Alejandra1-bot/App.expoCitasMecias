...
export default function EditarEspecialidad() {
  ...
  const handleGuardar = async () => {
    ...
    try {
      ...
      if (esEdicion) {
        result = await editarEspecialidad(especialidad.id, data);
      } else {
        result = await crearEspecialidad(data);
      }

      if (result.success) {
        Alert.alert("Éxito", esEdicion ? "Especialidad actualizada" : "Especialidad creada correctamente");
        navigation.goBack();
      } else {
